...
describe Registration do
  ...
  it "should work straight from the factory" do
    lambda {
      direct_registration_factory.should be_valid
  ...
  describe "conversions" do
    it "should accept a float for the amount and convert it to a BigDecimal interally" do
      registration = direct_registration_factory(:amount => 12.34)
    ...
    it "should accept an integer for the amount and convert it to a BigDecimal interally" do
      registration = direct_registration_factory(:amount => 12)
      registration.should be_valid
  ...
  describe "validations" do
    it { validates_the_presence_of(:direct_registration, :mode)             }
    it { validates_the_presence_of(:direct_registration, :tx_type)          }
    it { validates_the_presence_of(:direct_registration, :vendor)           }
    it { validates_the_presence_of(:direct_registration, :vendor_tx_code)   }
    it { validates_the_presence_of(:direct_registration, :amount)           }
    it { validates_the_presence_of(:direct_registration, :currency)         }
    it { validates_the_presence_of(:direct_registration, :description)      }
    it { validates_the_presence_of(:direct_registration, :card_details) }
    it { validates_the_presence_of(:direct_registration, :billing_address)  }
    it { validates_the_presence_of(:direct_registration, :delivery_address) }

    it { does_not_require_the_presence_of(:direct_registration, :customer_email)    }
    it { does_not_require_the_presence_of(:direct_registration, :basket)            }
    it { does_not_require_the_presence_of(:direct_registration, :allow_gift_aid)    }
    it { does_not_require_the_presence_of(:direct_registration, :apply_avs_cv2)     }
    it { does_not_require_the_presence_of(:direct_registration, :apply_3d_secure)   }
    it { does_not_require_the_presence_of(:direct_registration, :profile)           }
    it { does_not_require_the_presence_of(:direct_registration, :billing_agreement) }
    it { does_not_require_the_presence_of(:direct_registration, :account_type)      }

    it { validates_the_length_of(:direct_registration, :vendor,           :max => 15)    }
    it { validates_the_length_of(:direct_registration, :vendor_tx_code,   :max => 40)    }
    it { validates_the_length_of(:direct_registration, :currency,         :exactly => 3) }
    it { validates_the_length_of(:direct_registration, :description,      :max => 100)   }
    it { validates_the_length_of(:direct_registration, :customer_email,   :max => 255)   }
    it { validates_the_length_of(:direct_registration, :basket,           :max => 7500)  }

    it "should allow the amount to be a minimum of 0.01" do
      registration = direct_registration_factory(:amount => "0.01")
      registration.should be_valid

      registration = direct_registration_factory(:amount => "0.00")
      registration.should_not be_valid
      registration.errors[:amount].should include("is less than the minimum value (0.01)")

      registration = direct_registration_factory(:amount => "-23")
      registration.should_not be_valid
      registration.errors[:amount].should include("is less than the minimum value (0.01)")
    end

    it "should allow the amount to be a maximum of 100,000.00" do
      registration = direct_registration_factory(:amount => "100000")
      registration.should be_valid

      registration = direct_registration_factory(:amount => "100000.01")
      registration.should_not be_valid
      registration.errors[:amount].should include("is greater than the maximum value (100,000.00)")

      registration = direct_registration_factory(:amount => "123456")
      registration.should_not be_valid
      registration.errors[:amount].should include("is greater than the maximum value (100,000.00)")
    end

    it "should allow the transaction type to be one of :payment, :deferred or :authenticate" do
      registration = direct_registration_factory(:tx_type => :payment)
      registration.should be_valid

      registration = direct_registration_factory(:tx_type => :deferred)
      registration.should be_valid

      registration = direct_registration_factory(:tx_type => :authenticate)
      registration.should be_valid

      registration = direct_registration_factory(:tx_type => :chickens)
      registration.should_not be_valid
    ...
    it "should allow the mode to be one of :direct_simulator, :test or :live" do
      ...
      registration = direct_registration_factory(:mode => :live)
      registration.should be_valid

      registration = direct_registration_factory(:mode => :chickens)
      registration.should_not be_valid
      registration.errors[:mode].should include("is not in the list")
    end

    it "should allow the gift aid setting to be true or false" do
      registration = direct_registration_factory(:allow_gift_aid => true)
      registration.should be_valid

      registration = direct_registration_factory(:allow_gift_aid => false)
      registration.should be_valid

      registration = direct_registration_factory(:allow_gift_aid => "chickens")
      registration.should_not be_valid
    ...
    it "should allow apply_avs_cv2 to be 0 through 3 (see docs for what that means)" do
      registration = direct_registration_factory(:apply_avs_cv2 => 0)
      registration.should be_valid

      registration = direct_registration_factory(:apply_avs_cv2 => 1)
      registration.should be_valid

      registration = direct_registration_factory(:apply_avs_cv2 => 2)
      registration.should be_valid

      registration = direct_registration_factory(:apply_avs_cv2 => 3)
      registration.should be_valid

      registration = direct_registration_factory(:apply_avs_cv2 => 4)
      registration.should_not be_valid
    ...
    it "should allow apply_3d_secure to be 0 through 3 (see docs for what that means)" do
      registration = direct_registration_factory(:apply_3d_secure => 0)
      registration.should be_valid

      registration = direct_registration_factory(:apply_3d_secure => 1)
      registration.should be_valid

      registration = direct_registration_factory(:apply_3d_secure => 2)
      registration.should be_valid

      registration = direct_registration_factory(:apply_3d_secure => 3)
      registration.should be_valid

      registration = direct_registration_factory(:apply_3d_secure => 4)
      registration.should_not be_valid
      registration.errors[:apply_3d_secure].should include("is not in the list")
    end

    it "should allow profile to be normal or low" do
      registration = direct_registration_factory(:profile => :normal)
      registration.should be_valid

      registration = direct_registration_factory(:profile => :low)
      registration.should be_valid

      registration = direct_registration_factory(:profile => :chickens)
      registration.should_not be_valid
      registration.errors[:profile].should include("is not in the list")
    end

    it "should allow billing_agreement to be true or false" do
      registration = direct_registration_factory(:billing_agreement => true)
      registration.should be_valid

      registration = direct_registration_factory(:billing_agreement => false)
      registration.should be_valid

      registration = direct_registration_factory(:billing_agreement => "chickens")
      registration.should_not be_valid
    ...
    it "should allow the account type to be one of ecommerce, continuous authority or mail order" do
      registration = direct_registration_factory(:account_type => :ecommerce)
      registration.should be_valid

      registration = direct_registration_factory(:account_type => :continuous_authority)
      registration.should be_valid

      registration = direct_registration_factory(:account_type => :mail_order)
      registration.should be_valid

      registration = direct_registration_factory(:account_type => :chickens)
      registration.should_not be_valid
  ...
  describe "url generation" do
    ...
    it "should pick the correct url for the test mode" do
      registration = direct_registration_factory(:mode => :test)
      registration.url.should == "https://test.sagepay.com/gateway/service/vspdirect-register.vsp"
    end

    it "should pick the correct url for the live mode" do
      registration = direct_registration_factory(:mode => :live)
      registration.url.should == "https://live.sagepay.com/gateway/service/vspdirect-register.vsp"
    end

    it "should raise an error if the mode has been set to something dodgy" do
      lambda {
        registration = direct_registration_factory(:mode => :dead_chickens)
        registration.url
  ...
  describe "post params generation" do
    context "given one or more invalid parameters" do
      it "should raise an error when trying to generate the URL" do
        registration = direct_registration_factory(:tx_type => :invalid)
    ...
    context "given the minimum set of required parameters" do
      ...
      describe "the posted params" do
        ...
        it "should contain the card holder name" do
          @registration.post_params["CardHolder"].should == "Card names"
        end

        it "should contain the card nuber" do
          @registration.post_params["CardNumber"].should == "5404000000000001"
        end

        it "should contain the expiry date" do
          @registration.post_params["ExpiryDate"].should == "0615"
        end

        it "should contain the card type" do
          @registration.post_params["CardType"].should == "MC"
        end
  ...
  describe "#signature_verification_details" do
    before(:each) do
      ...
      @registration = direct_registration_factory :vendor_tx_code => "vendor transaction id", :vendor => "vendor"
